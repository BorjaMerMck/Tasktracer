...
class _AjustesScreenState extends State<AjustesScreen> {
  ...
  List<String> userHomeGroups = []; // List to store all groups
  ...
  Future<void> _fetchUserGroups() async {
    ...
    if (user != null) {
      try {
        // Fetch all home groups where the user is a member
  ...
  Future<void> _fetchHomeMembers(String homeName) async {
    try {
      final homeDoc = await _firestore.collection('homes').where('homeName', isEqualTo: homeName).limit(1).get();
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    // Dropdown for selecting home group
                    DropdownButtonFormField<String>(
                      value: selectedHomeGroup,
                      items: userHomeGroups.map((group) {
